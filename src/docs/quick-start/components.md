...
You'll notice that we've added some props to our `Input`. DRY's components were designed to accept and render any valid React/default HTML prop. Components that are based on HTML primitives, like `Input` or `Select`, will work just like a regular `input` or `select` (except better 😉).
...
The `Input` spacing seems a little off in our `Form` 😱. Good thing DRY has a `FormGroup` component to help with this very thing!
...
Nice 😍! Our `Form` is looking mighty spiffy. It's a good thing DRY's components are designed to work with each other. This allows you to compose your UI however it is you see fit.
...
Let's start [contributing](../contributing/setup.md)!
